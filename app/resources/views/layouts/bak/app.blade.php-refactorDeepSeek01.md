...
<body class="font-sans antialiased bg-[var(--bg-primary)] text-[var(--text-primary)] transition-colors duration-300">

    {{-- Contenedor padre --}}
    <div class="h-screen bg-[var(--bg-primary)] flex flex-col lg:flex-row overflow-hidden transition-colors duration-300">
        ...
        <aside id="sidebar" class="fixed lg:relative inset-y-0 left-0 w-56 lg:w-56 bg-brand-glow text-white transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out z-50 flex-shrink-0 h-full overflow-y-auto lg:block">
            <div class="h-full flex flex-col">
                ...
                <div class="px-4 pb-4 max-h-48 overflow-y-auto">
                    <h3 class="text-xs font-semibold text-brand-muted uppercase tracking-wider mb-2">Filtrar por categoría</h3>
                ...
                @if(Auth::check() && Auth::user()->is_admin)
                    <div class="px-4 pb-4 border-t border-brand-glow/40 mt-2 pt-4">
                        <h3 class="text-xs font-semibold text-brand-muted uppercase tracking-wider mb-2">Administración</h3>
                        <div class="space-y-1">
        ...
        {{-- Main Content --}}
        <main class="flex-1 overflow-y-auto bg-[var(--bg-primary)] transition-colors duration-300">
            @if(session('success'))
    ...
    <div id="tagsModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
        <div class="bg-[var(--bg-card)] rounded-lg shadow-xl max-w-md w-full mx-4 flex flex-col max-h-[90vh] border border-[var(--border-color)] transition-colors duration-300">
            <div class="p-6">
                <h3 class="text-lg font-semibold text-[var(--text-primary)] mb-4"># Editar etiquetas</h3>
                
                <div class="mb-4">
                    <div class="flex gap-2">
                        <input type="text" id="newTagName" placeholder="Nueva etiqueta" 
                            class="flex-1 px-4 py-2 rounded-lg border border-[var(--border-color)] bg-[var(--bg-input)] text-[var(--text-primary)] focus:outline-none focus:border-brand-glow focus:ring-1 focus:ring-brand-glow transition-colors">
                ...
                <div id="tagsList" class="space-y-2 max-h-64 overflow-y-auto flex-1 max-h-64">
    ...
    <script>
            ...
            fetch('{{ route("tags.index") }}')
                ...
                .then(tags => {
                    ...
                    tags.forEach(tag => {
                        ...
                        div.className = 'flex items-center justify-between p-2 bg-[var(--bg-input)] rounded-lg transition-colors';
                        div.innerHTML = `
                            <span class="text-[var(--text-primary)]">🏷️ ${tag.name}</span>
        ...
        // Sincronizar tema con preferencia del usuario (para el auto)
        // Esto ya se maneja en el HTML con PHP, pero aquí está la lógica JavaScript
        // para cambios en tiempo real si fuera necesario.
    </script>
