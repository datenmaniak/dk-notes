<x-app-layout>
    <div class="p-6 pt-16 lg:pt-6 max-w-4xl mx-auto">
        ...
        <div class bg-daten-card rounded-xl border border-[rgba(119,0,240,0.15)] shadow-sm p-6 transition-all duration-300">
            ...
            <div class="mb-6">
                ...
                <a href="{{ route('notes.show', ['note' => $note] + $queryParams) }}"
                   class="text-sm font-medium text-slate-600 border border-[#7700F0]/2 bg-daten-card hover:bg-[#7700F0]/10 hover:text-[#7700F0] px-3 py-2 inline-flex items-center gap-2 transition-all duration-200 rounded-lg group">
                    <i class="fa-solid fa-arrow-left text-xs text-slate-400 group-hover:text-[#7700F0] transition-colors"></i> 
                    Volver a la nota
                </a>
            </div>
            
            {{-- Cabecera de la sección - Aumentado a text-3xl --}}
            <div class="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
                <div class="w-12 h-12 rounded-lg bg-[#7700F0]/10 flex items-center justify-center text-[#7700F0]">
                    <i class="fa-solid fa-pen-to-square text-xl"></i>
                </div>
                <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Editar Nota</h1>
            </div>
            ...
            <form method="POST" action="{{ route('notes.update', $note) }}">
                ...
                <div class="flex gap-3 justify-end mb-6">
                    {{-- Botón Cancelar (Fondo blanco) actualizado con borde primario claro y hover de marca --}}
                    <a href="{{ route('notes.show', ['note' => $note] + $queryParams) }}" 
                       class="px-4 py-2 border border-[#7700F0]/20 text-slate-70 bg-daten-card hover:bg-[#7700F0]/10 hover:text-[#7700F0] rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 active:scale-95">
                        <i class="fa-solid fa-ban text-slate-400 group-hover:text-[#7700F0]"></i> Cancelar
                    </a>
                    <button type="submit" 
                            class="px-4 py-2 bg-[#7700F0] hover:bg-[#8616ff] text-white rounded-lg text-sm font-medium shadow-md shadow-[#7700F0]/10 hover:shadow-lg hover:shadow-[#7700F0]/25 transition-all duration-200 flex items-center gap-2 active:scale-95">
                ...
                <div class="mb-5 space-y-2">
                    ...
                    <input type="text" name="title" value="{{ old('title', $note->title) }}" 
                           class="w-full px-4 py-3 rounded-lg border border-slate-200 bg-slate-50/80 text-sm text-slate-800 focus:outline-none focus:border-[#7700F0] focus:ring-4 focus:ring-[#7700F0]/15 transition-all duration-200" required>
                </div>
                ...
                <div class="mb-5 space-y-2">
                    ...
                    <select name="category_id" 
                            class="w-full px-4 py-3 rounded-lg border border-slate-200 bg-slate-50/80 text-sm text-slate-800 focus:outline-none focus:border-[#7700F0] focus:ring-4 focus:ring-[#7700F0]/15 transition-all duration-200">
                ...
                <div class="mb-5 bg-slate-50/50 rounded-xl border border-dashed border-slate-200 p-4">
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        ...
                        {{-- Botón Administrar (Fondo blanco) actualizado con borde primario claro y hover de marca --}}
                        <button type="button" onclick="openAssignTagsModal()"
                                class="px-3 py-1.5 text-xs text-daten-secondary border border-[#7700F0]/2 bg-daten-card rounded-lg hover:bg-[#7700F0]/10 hover:text-[#7700F0] focus:ring-4 focus:ring-[#7700F0]/15 transition-all duration-200 font-medium flex items-center gap-1.5">
                ...
                <div class="mb-6 space-y-2">
                    ...
                    <textarea name="content_markdown" rows="15" 
                              class="w-full p-4 rounded-lg border border-slate-200 bg-slate-50/80 font-mono text-sm text-slate-800 focus:outline-none focus:border-[#7700F0] focus:ring-4 focus:ring-[#7700F0]/15 transition-all duration-200" required>{{ old('content_markdown', $note->content_markdown) }}</textarea>
                </div>
                ...
                <div id="assignTagsModal" class="hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center transition-all">
                    <div class bg-daten-card rounded-xl border border-[rgba(119,0,240,0.15)] shadow-2xl max-w-md w-full mx-4 overflow-hidden">
                        {{-- Título interno del modal aumentado a text-xl --}}
                        <div class="p-6 border-b border-slate-100 flex items-center gap-2.5 text-slate-900">
                            <i class="fa-solid fa-tags text-[#7700F0] text-xl"></i>
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
                                {{-- Cancelar Interno (Fondo blanco) con borde primario claro y hover de marca --}}
                                <button type="button" onclick="closeAssignTagsModal()" 
                                        class="px-4 py-2 border border-[#7700F0]/20 text-slate-70 bg-daten-card hover:bg-[#7700F0]/10 hover:text-[#7700F0] rounded-lg text-sm font-medium transition-all duration-200">
                                    Cancelar
                                </button>
                                <button type="button" onclick="saveAssignedTags()" 
                                        class="px-4 py-2 bg-[#7700F0] hover:bg-[#8616ff] text-white rounded-lg text-sm font-medium shadow-md shadow-[#7700F0]/10 hover:shadow-lg hover:shadow-[#7700F0]/25 transition-all duration-200">
                        ...
                        fetch('{{ route("tags.index") }}')
                            ...
                            .then(tags => {
                                ...
                                container.innerHTML = allTags.map(tag => `
                                    <label class="flex items-center gap-3 p-2.5 hover:bg-[#7700F0]/10 rounded-lg cursor-pointer border border-transparent hover:border-[#7700F0]/15 transition-all group">
                                        ...
                                        <div class="flex items-center gap-2 text-sm font-medium text-slate-700 group-hover:text-slate-900">
                                            <i class="fa-solid fa-tag text-slate-300 group-hover:text-[#7700F0]/60 transition-colors text-xs"></i>
                                            <span>${tag.name}</span>
                                        </div>
                                    </label>
                                `).join('');
