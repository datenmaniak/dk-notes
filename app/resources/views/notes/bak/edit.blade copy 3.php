        ...
        <div class="bg-white rounded-xl border border-[rgba(119,0,240,0.15)] shadow-sm p-6 transition-all duration-300">
            ...
            <div class="mb-6">
                ...
                <a href="{{ route('notes.show', ['note' => $note] + $queryParams) }}"
                   class="text-sm font-medium text-slate-600 hover:text-[#7700F0] hover:bg-purple-50/50 px-3 py-2 inline-flex items-center gap-2 transition-all duration-200 rounded-lg group">
            ...
            <form method="POST" action="{{ route('notes.update', $note) }}">
                ...
                <div class="flex gap-3 justify-end mb-6">
                    <a href="{{ route('notes.show', ['note' => $note] + $queryParams) }}" 
                       class="px-4 py-2 border border-slate-200 text-slate-700 bg-white hover:border-[#7700F0]/30 hover:bg-purple-50/30 hover:text-[#7700F0] rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-2 active:scale-95">
                ...
                <div class="mb-5 bg-slate-50/50 rounded-xl border border-dashed border-slate-200 p-4">
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        ...
                        <button type="button" onclick="openAssignTagsModal()"
                                class="px-3 py-1.5 text-xs text-[#7700F0] border border-[#7700F0]/30 bg-white rounded-lg hover:border-[#7700F0]/60 hover:bg-purple-50 focus:ring-4 focus:ring-[#7700F0]/15 transition-all duration-200 font-medium flex items-center gap-1.5">
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex justify-end gap-3 mt-6 pt-4 border-t border-slate-100">
                                <button type="button" onclick="closeAssignTagsModal()" 
                                        class="px-4 py-2 border border-slate-200 text-slate-700 bg-white hover:bg-slate-50 rounded-lg text-sm font-medium transition-all duration-200">
                                    Cancelar
                                </button>
                        ...
                        fetch('{{ route("tags.index") }}')
                            ...
                            .then(tags => {
                                ...
                                container.innerHTML = allTags.map(tag => `
                                    <label class="flex items-center gap-3 p-2.5 hover:bg-purple-50/40 rounded-lg cursor-pointer border border-transparent hover:border-[#7700F0]/15 transition-all group">
