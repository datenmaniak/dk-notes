...
show_help() {
  ...
  echo "  -i, --image <nombre>       Nombre de la imagen (ej: backend-api)"
  echo "  -t, --tag <version>        Tag/versión (ej: v1.2.3)"
  echo "  -f, --dockerfile <ruta>    Ruta al Dockerfile (ej: ./Dockerfile)"
  echo ""
  echo "Opciones de control:"
  echo "  -b, --build-only           Solo construir imagen (no subir)"
  echo "  -p, --push-only            Solo subir imagen existente (no construir)"
  echo ""
  echo "Otras opciones:"
  echo "  -n, --no-cache             Construir sin usar caché"
  echo "  -a, --build-arg <key=val>  Argumentos para el build (puede repetirse)"
  echo "  -v, --verbose              Mostrar comandos ejecutados"
  echo "  -h, --help                 Mostrar esta ayuda"
  echo ""
  echo "Comportamiento por defecto: Build + Push"
  echo ""
  echo "Ejemplos:"
  echo "  $0 -i api -t v1.0.0 -f ./Dockerfile"
  echo "  $0 -i api -t v1.0.0 -f ./Dockerfile -b"
  echo "  $0 -i api -t v1.0.0 -f ./Dockerfile -p"
  echo "  $0 -i api -t v1.0.0 -f ./Dockerfile -n -a ENV=prod -a DEBUG=false"
  echo ""
...
if [ $# -gt 0 ] && [[ ! "$1" =~ ^-- ]] && [[ ! "$1" =~ ^- ]]; then
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
    -b | --build-only) DO_PUSH=false ;;
    -p | --push-only) DO_BUILD=false ;;
...
if [ "$DO_BUILD" = false ] && [ "$DO_PUSH" = false ]; then
  echo "❌ -b/--build-only y -p/--push-only son mutuamente excluyentes"
  exit 1
fi
...
if [ "$DO_PUSH" = true ] && [ "$DO_BUILD" = false ]; then
  ...
  if ! podman image inspect "$FULL_IMAGE" >/dev/null 2>&1; then
    ...
    echo "   Sugerencia: Ejecuta sin -p/--push-only para construirla primero"
    exit 1
  fi
...
#   if ! echo "$REGISTRY_PASS" | podman login "$REGISTRY_URL" -u "$REGISTRY_USER" -p "$REGISTRY_PASS" --password-stdin --tls-verify=false >/dev/null 2>&1; then
...
if [ "$DO_PUSH" = true ]; then
    # Autenticar solo si se va a hacer push
    echo "🔐 Autenticando en: $REGISTRY_URL"
    log_verbose "echo \"$REGISTRY_PASS\" | podman login $REGISTRY_URL -u $REGISTRY_USER --password-stdin --tls-verify=false"
    
    # if ! echo "$REGISTRY_PASS" | podman login "$REGISTRY_URL" -u "$REGISTRY_USER" --password-stdin --tls-verify=false > /dev/null 2>&1; then
    #     echo "❌ Error en autenticación con el registro"
    #     exit 1
    # fi
    # echo "✅ Autenticación exitosa"

    # Ejecutar login y capturar salida
LOGIN_OUTPUT=$(echo "$REGISTRY_PASS" | podman login "$REGISTRY_URL" -u "$REGISTRY_USER" --password-stdin --tls-verify=false 2>&1)
LOGIN_EXIT_CODE=$?

# Verificar código de salida
if [ $LOGIN_EXIT_CODE -ne 0 ]; then
    echo "❌ Error en autenticación con el registro"
    echo "   Detalle: $LOGIN_OUTPUT"
    exit 1
fi

# Verificar mensaje de éxito (opcional)
if echo "$LOGIN_OUTPUT" | grep -q "Login Succeeded"; then
    echo "✅ Autenticación exitosa"
else
    echo "⚠️ Login completado pero no se confirmó éxito"
    echo "   Detalle: $LOGIN_OUTPUT"
fi
