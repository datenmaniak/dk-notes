...
<body class="font-sans antialiased bg-(--bg-primary) text-(--text-primary) transition-colors duration-300">
    ...
    {{-- Contenedor padre --}}
    <div class="h-screen bg-(--bg-primary) flex flex-col lg:flex-row overflow-hidden transition-colors duration-300">
        ...
        {{-- Main Content --}}
        <main class="flex-1 overflow-y-auto bg-(--bg-primary) transition-colors duration-300">
            @if(session('success'))
    ...
    <div id="tagsModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
        <div class="bg-(--bg-card) rounded-lg shadow-xl max-w-md w-full mx-4 flex flex-col max-h-[90vh] border border-(--border-color) transition-colors duration-300">
            <div class="p-6">
                ...
                <div class="mb-4">
                    <div class="flex gap-2">
                        <input type="text" id="newTagName" placeholder="Nueva etiqueta" 
                            class="flex-1 px-4 py-2 rounded-lg border border-(--border-color) bg-(--bg-input) text-(--text-primary) focus:outline-none focus:border-brand-glow focus:ring-1 focus:ring-brand-glow transition-colors">
            ...
            fetch('{{ route("tags.index") }}')
                ...
                .then(tags => {
                    ...
                    tags.forEach(tag => {
                        ...
                        div.innerHTML = `
                            <span class="text-[var(--text-primary)]"><i class="fas fa-tag"></i> ${tag.name}</span>
